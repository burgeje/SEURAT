 ...
 *
 */
public class DecisionEditor extends RationaleEditorBase {
	public static RationaleEditorInput createInput(RationaleExplorer explorer, TreeParent tree,
		...
		return new DecisionEditor.Input(explorer, tree, parent, target, new1);
	}
	/**
	...
	public Decision getDecision() {
		return (Decision)getEditorData().getAdapter(Decision.class);
	}
	...
	public void loadDesignerLabel()
	{
		...
		{
			designerLabel.setText("No designer is associated with this Decision");		
		}
	 ...
	 */
	public void onUpdate(Decision pElement, RationaleUpdateEvent pEvent)
	{
		try
		{
		...
		{
			System.out.println("Exception in DecisionEditor: onUpdate");
		}
	}
	...
	public void refreshAlternatives(RationaleUpdateEvent pEvent)
	{
		...
		while (iterator.hasNext())
		{
			...
			{
				/* molerjc- changed this to onAlternativeUpdate, was onForArgumentUpdate which was throwing an exception
				* and not registering the function */
				subscribeFunc = "onAlternativeUpdate";				
			}
		} 
		...
		try
		{
			...
			// molerjc- check here to make sure it's an alternative; if it's an argument don't try to subscribe to it
			if (pEvent != null  && pEvent.getElement().getElementType() == RationaleElementType.ALTERNATIVE)
			{
...
//				System.out.println(ourDec.getPhase().toString());
		...
		while (altI.hasNext())
		{
			...
			{
				System.out.println("Decision Editor: Alternative Update Not Available!");
			}
		}    
	...
	public boolean saveData() {
		ConsistencyChecker checker = new ConsistencyChecker(getDecision().getID(), nameField.getText(), "Decisions");
		
		if(!nameField.getText().trim().equals("") &&
				(getDecision().getName() == nameField.getText() || checker.check(false))) {
			if (isCreating()) {
				...
				if (!newStat.toString().equals(getDecision().getStatus().toString())) {
						...
						newHist.toDatabase(getDecision().getID(), RationaleElementType.DECISION);
				}
		...
		{
			String l_message = "";
			l_message += "The decision name you have specified is either already"
				+ " in use or empty. Please make sure that you have specified"
				+ " a decision name and the decision name does not already exist"
				+ " in the database.";
			MessageBox mbox = new MessageBox(getSite().getShell(), SWT.ICON_ERROR);
			mbox.setMessage(l_message);
			mbox.setText("Decision Name Is Invalid");
			mbox.open();
		}
		return false;
	}
	...
	public static class Input extends RationaleEditorInput {
		 ...
		 */
		public Decision getData() { return (Decision)getAdapter(Decision.class); }
		...
		public String getName() {
			return isCreating() ? "New Decision Editor" :
				"Decision: " + getData().getName();
		}
