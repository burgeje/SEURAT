 ...
 */
public class SelectRationaleElement {
	 ...
	 */
	public SelectRationaleElement(Display display, RationaleElementType eType) {
...
//		new Label(shell, SWT.NONE).setText("Entity Type:");
		...
		Vector listV = db.getRationaleElementList(type);
...
//		gridData.horizontalIndent = 5;
		...
		if (eType==RationaleElementType.ALTERNATIVE)
			type=RationaleElementType.DECISION;
		if (eType==RationaleElementType.DECISION)
		...
		editB.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				...
				ourItem  = RationaleDB.getRationaleElement(name, type);				
				shell.close();
				shell.dispose();
			}
		});
