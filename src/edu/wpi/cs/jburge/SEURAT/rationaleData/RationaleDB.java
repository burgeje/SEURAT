...
public final class RationaleDB implements Serializable {
	...
	/**
	 * Get a list of alternatives GENERATED by generate pattern wizard. Only those patterns belong to
	 * the category will be added to the vector.
	 * @param category The pattern category of the alternative.
	 * @return A vector of alternatives generated from pattern generation wizard.
	 */
	 ...
	 * Get all ontology at a particular level of the tree.
	 * @param entries
	 * @param curLevel
	 * @param targetLevel
	 * @return null if entries is empty. Otherwise, returns a vector containing all ontology elements at a particular level.
	 */
	private Vector<OntEntry> getOntologiesAtLevel(Vector<OntEntry> entries, int curLevel, int targetLevel){
	...
	/**
	 * Get a vector of all ontologies at a level under the subroot.
	 * NOTE: the level of the subroot is 0.
	 * @param subroot
	 * @param targetLevel
	 * @return null if subroot == null. Otherwise, return a vector containing all elements that are targetLevel
	 * down to the tree from subroot.
	 */
	public Vector<OntEntry> getOntologiesAtLevel(OntEntry subroot, int targetLevel){
	 ...
	 * @return -1 if desc is not a descendant of parent. Otherwise, returns the relative sublevel of 
	 * descendant to the parent.
	 */
	public int findRelativeOntLevel(OntEntry parent, OntEntry desc){
		Vector<OntEntry> cur = new Vector<OntEntry>();
		cur.add(parent);
		if (parent.equals(desc)) return 0;
		int level = 1;
		Vector<OntEntry> next = getOntologiesAtLevel(parent, level);
		while (!cur.equals(next)){
			cur = next;
			if (cur.contains(desc)) return level;
			level++;
