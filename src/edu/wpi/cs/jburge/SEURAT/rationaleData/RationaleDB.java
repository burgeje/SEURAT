 ...
 * operations. If you need to get things from the DB in bulk, this is the one to
 * call. The class is set up as a singleton design pattern.
 * 
 * @author burgeje
 * 
 */
public final class RationaleDB implements Serializable {
	...
	/**
	 * If you're loading up the DB from an XML file, this is where the name is
	 * stored.
	 */
	...
	/**
	 * Contains the last rationale ID. I'm not sure what this is used for - I'd
	 * guess maybe the XML?
	 */
	...
	/**
	 * Holds the root of the argument ontology when reading in from XML (This is
	 * used in the old readXMLData method, not the new argument ontology import
	 * functionality.)
	 */
	 ...
	 */
	// private static String ratDBCreateFile =
	// SEURATPlugin.getDefault().getStateLocation()
	// .addTrailingSeparator().toOSString() + "ratDBCreate.xml";
	private static URL ratDBPath = SEURATPlugin.getDefault().getBundle()
			.getEntry("/");
	/**
	...
	private PublishSubscribeManager<RationaleElement, Object, RationaleUpdateEvent> updateNotifier = new PublishSubscribeManager<RationaleElement, Object, RationaleUpdateEvent>(
			new RationaleUpdateEvent(), false);

	public PublishSubscribeManager<RationaleElement, Object, RationaleUpdateEvent> Notifier() {
		return updateNotifier;
	}

	/**
	 * Rationale DB Constructor. This MUST remain PRIVATE in order for this
	 * class to stay a d. This class sets up the JDBC database connection using
	 * the information from the preferences.
	 * 
	 * @param x
	 *            - unused parameter.
	 */
	private RationaleDB(int x) {
		...
		// This hashtable's second element is String[3], with String[1] as
		// problemcategory
		// and String[2] as type (Architecture, Design, Idiom), and String[0] as
		// id in the database.
	 ...
	 */
	private void sqlConnect() { // Set up our database connection
		...
		String password = store
				.getString(PreferenceConstants.P_DATABASEPASSWORD);
		...
		// Set up connection string based on server location, address, and port
		// preferences
		if (store.getString(PreferenceConstants.P_MYSQLLOCATION).equals(
				PreferenceConstants.MySQLLocationType.LOCAL)) {
			connStr += "localhost/";
		} else {
			String address = store
					.getString(PreferenceConstants.P_MYSQLADDRESS);
		...
		connStr += dbName + "?user="
				+ store.getString(PreferenceConstants.P_DATABASEUSER)
				+ passwordSpec;
	...
	/**
	 * Rationale DB connect method for an Apache Derby local database. This
	 * method is called from the RationaleDB constructor if the SEURAT
	 * preferences specify Derby.
	 */
	private void derbyConnect() {
		...
		try {
			...
			l_DerbyPathType = store
					.getString(PreferenceConstants.P_DERBYPATHTYPE);
		} catch (Exception ex) {
			Shell shell = new Shell();
			MessageBox mbox = new MessageBox(shell, SWT.ICON_ERROR);
			mbox.setMessage("Error getting preferences");
		...
		String l_dbPath = "";
		if (l_DerbyPathType
				.equals(PreferenceConstants.DerbyPathType.ABSOLUTE_PATH)) {
			l_dbPath += l_DerbyPath;
		} else {
			l_dbPath += ResourcesPlugin.getWorkspace().getRoot().getLocation()
					.toOSString();
		}
		l_dbPath += "/" + dbName;
		...
		try {
			...
			try {
				...
				reportError(eInner, "constructor",
						"couldn't connect to database " + dbName);
				Shell shell = new Shell();
				MessageBox mbox = new MessageBox(shell, SWT.ICON_ERROR);
				mbox.setMessage("Couldn't connect to database " + dbName);
				mbox.open();
	...
	/**
	 * Rationale DB method to create the initial tables for a new rationale
	 * database. This method is called by the connect methods if a new database
	 * needs to be created, and functions by retrieving and executing queries
	 * using the RationaleDBCreate class.
	 * 
	 * @throws SQLException
	 */
	private void createTables() throws SQLException {
		...
		try {
			ratDBCreateURL = FileLocator.resolve(new URL(ratDBPath,
					"ratDBCreate.xml"));
		...
		if (!importXMLSuccess) {
			...
			// Import hard-coded data, first the ontology, then the pattern
			// library...
			...
			for (String l_ontQuery : l_ontQueries) {
				try {
					...
					i++;
					System.out
							.println("DEBUG OntEntry: " + eError.getMessage());
			...
			PreparedStatement ps = conn
					.prepareStatement("INSERT INTO patterns (name, type, description,problem, context, solution, implementation,example,url,id) values (?,?,?,?,?,?,?,?,?,?)");
			importPatterns(ps);
			ps.close();

			RationaleDBCreate.resetCurrentID();
			String[] l_patternDecisionQueries = RationaleDBCreate
					.getPatternDecisionQueries();
			for (String l_patternDecisionQuery : l_patternDecisionQueries) {
				try {
					...
					int i = 0;
					i++;
					System.err.println("Debug PatternDecision: "
							+ e.getMessage());
					throw e;
				}
			}
			...
			RationaleDBCreate.resetCurrentID();
			String[] l_patternOntEntryQueries = RationaleDBCreate
					.getPatternOntEntryQueries();
			for (String l_patternOntEntryQuery : l_patternOntEntryQueries) {
				try {
					...
					int i = 0;
					i++;
					System.err.println("Debug PatternOntEntry: "
							+ e.getMessage());
					throw e;
				}
			}
			...
			RationaleDBCreate.resetCurrentID();
			String[] l_patternDecisionRelationshipQueries = RationaleDBCreate
					.getPatternDecisionRelationShipQueries();
			...
			RationaleDBCreate.resetCurrentID();
			String[] l_patternProblemCategoriesQueries = RationaleDBCreate
					.getPatternProblemCategoriesQueries();
			...
			RationaleDBCreate.resetCurrentID();
			String[] l_patternProblemCategoryQueries = RationaleDBCreate
					.getPatternProblemCategoryQueries();
	...
	/**
	 * Prepared Statement Which Is Used Primarily By The Select Ontology Dialog
	 * To Generate A Tree Of Ontology Elements To Select.
	 * 
	 * @see SelectOntEntry
	 */
	...
	/**
	 * Prepared Statement For The Ontology Entry's FromDatabase pseudo
	 * constructor.
	 * 
	...
	/**
	 * Accessor Method For The Ontology Entries FromDatabase Pseudo Constructor.
	 * This is how the Ontology Entry Class Retrieves The Prepared Statement
	 * 
	 * @see OntEntry#fromDatabase(String)
	 * 
	 * @return A prepared statement for retrieving an ontology entry from the
	 *         database by name.
	 */
	...
	/**
	 * Accessor method. Retrieves the prepared statement for pattern
	 * alternatives
	 * 
	 * @return
	 */
	...
	/**
	 * Generate prepared statements for use in database queries. The Prepared
	 * Statements Make accessing The Database Much Faster.
	 */
	private void prepareStatements() {
		...
		try {
			...
			m_ontEntryFromDB = conn
					.prepareStatement("SELECT id, importance, description "
							+ " from "
		...
		try {
			...
			m_patternFromDB = conn.prepareStatement("SELECT * " + " from "
					+ RationaleDBUtil.escapeTableName("patterns")
					+ " where name = ?");
			...
			m_altPatternFromDB = conn.prepareStatement("SELECT * " + " from "
					+ RationaleDBUtil.escapeTableName("alternativepatterns")
					+ " where name = ?");
	...
	/**
	 * This Code Seamlessly Updates A Database With New Data. It will check the
	 * existing database schema and determine whether the desired database
	 * properties exist. If the properties do not exist then the database schema
	 * will be modified to contain the new attributes with default values.
	 * 
	...
	private void migrate() {
		...
		try {
			...
			l_stmt.executeQuery("SELECT * FROM "
					+ RationaleDBUtil.escapeTableName("ONT_HIERARCHY"));
		} catch (Exception eError) {
			...
			try {
				...
				l_create.execute(RationaleDBCreate
						.CREATEVIEW_ONTRELATIONSHIP_HIERARCHY());
	...
	/**
	 * Reset our connection to the database. This is done when the user decides
	...
	/**
	 * Create a new database using the SEURAT_Base database as a start. This is
	 * needed so we get our empty rationale element tables and a fully populated
	 * Argument Ontology
	 * 
	 * @param newDBName
	 *            - the name for our new database.
	 */
	...
	/**
	 * Given the name and type of a rationale element (such as an argument),
	 * return find all requirements that are children of that element in the
	 * database.
	 * 
	 * @param parentName
	 *            - name of the parent element in the tree
	 * @param parentType
	...
	/**
	 * Given the name and type of a rationale element , return find all
	 * contingencies that are children of that element in the database.
	 * 
	 * @param parentName
	 *            - name of the parent element in the tree
	 * @param parentType
	...
	/**
	 * Given the name and type of a rationale element , return find all
	 * designers that are children of that element in the database.
	 * 
	 * @param parentName
	 *            - name of the parent element in the tree
	 * @param parentType
	...
	public Vector<Association> getAssociations() {
		...
		try {
			...
			while (rs.next()) {
				Association as = new Association(rs.getInt("alternative"),
						rs.getString("artifact"), rs.getString("artresource"),
						rs.getString("artname"), rs.getString("assocmessage"));
				assocs.add(as);
			}
		} catch (SQLException ex) {
			// handle any errors
	...
	/**
	 * Get all the requirements from the database that have a particular status
	 * value (Addressed, Satisfied, or Violated)
	 * 
	 * @param stat
	 *            the status value we are looking for
	...
	/**
	 * Get all the requirements from the database that are either enabled or
	 * disabled, depending on what we are looking for
	 * 
	 * @param enabled
	 *            the enabled value we are looking for (true/false)
	 * @return a vector of requirements
	 */
	...
	/**
	 * Get all the decisions that correspond to a particular parent and parent
	 * type.
	 * 
	 * @param parentName
	 *            - the name of our parent element
	 * @param parentType
	 *            - the parent type
	...
	/**
	 * Return a list of alternatives that are depent on the passed in
	 * alternative
	 * 
	 * @param alt
	 *            - our alternative
	...
	public Vector<Alternative> getDependentAlternatives(Alternative alt) {
		...
		try {
			...
			findQuery = "SELECT parent, ptype from "
					...
					+ " and (type = 'Pre-supposes' or type = 'Opposed' or type = 'Complicates' or type = 'Facilitates')";
			// *** System.out.println(findQuery);
			rs = stmt.executeQuery(findQuery);
			while (rs.next()) {
	...
	/**
	 * Return a list of alternatives that are dependent on the given pattern
	 * alternative.
	 * 
	 * @param alt
	 * @param atype
	 * @return
	 */
	public Vector<Alternative> getDependentAlternatives(AlternativePattern alt,
			ArgType atype) {
		Vector<Alternative> dependent = new Vector<Alternative>();
		Statement stmt = null;
		ResultSet rs = null;
		...
		try {
			...
			findQuery = "SELECT parent, ptype from "
					+ RationaleDBUtil.escapeTableName("arguments")
					+ " where alternative = "
					+ new Integer(alt.getID()).toString() + " and (type = '"
					+ atype.toString() + "')";
			// *** System.out.println(findQuery);
			rs = stmt.executeQuery(findQuery);
			while (rs.next()) {
	...
	public static Vector<Argument> getDependentArguments(int reqID) {
		...
		try {
			...
			findQuery = "SELECT id from "
					+ RationaleDBUtil.escapeTableName("arguments")
					+ " where argtype = 'Requirement'" + " and requirement = "
			...
			reportError(ex,
					"Error in getting arguments that argue the requirement",
					findQuery);
		} finally {
			releaseResources(stmt, rs);
	...
	public static Vector<Argument> getDependentAssumptionArguments(int reqID) {
		...
		try {
			...
			findQuery = "SELECT id from "
					+ RationaleDBUtil.escapeTableName("arguments")
					+ " where argtype = 'Assumption" + " and assumption = "
					+ new Integer(reqID).toString();
			// *** System.out.println(findQuery);
			rs = stmt.executeQuery(findQuery);
			while (rs.next()) {
	...
	public Vector<TreeParent> getAlternatives(String parentName,
		...
		Vector<TreeParent> altTree = getTreeElements(parentName,
				"alternatives", parentType);
		if (altTree != null) {
			Statement stmt = null;
			ResultSet rs = null;
			try {
				...
				while (alts.hasMoreElements()) {
					...
					findQuery = "SELECT status from "
							...
							+ " where " + "name = '"
							+ RationaleDBUtil.escape(alt.getName()) + "'";
					rs = stmt.executeQuery(findQuery);
					if (rs.next()) {
	...
	/**
	 * Given the type of RationaleElement (RationaleElementType), return the
	 * name of the database table that contains elements of that type.
	 * 
	 * @param element
	 *            - the element type
	...
	public RationaleElementType getElementType(String elementType) {
		...
		/*
		 * else if (elementType.compareTo("requirements" == 0)) { return
		 * ("tradeoffs"); }
		 */
		else {
	...
	public String getAlternativePattern(String parentName) {
		String alternativePattern = getElement(parentName, "arguments",
				"alternativepattern", RationaleElementType.ALTERNATIVEPATTERN);
	...
	/**
	 * Given the name of the parent, the table type of the parent, the name of
	 * the element field in the parent table, and the element type, get the name
	 * of the element. Ok, this is confusing but the parent table has the ID of
	 * the item associated with it, not its name. As an example, you want the
	 * ontology entry associated with a claim called "Cheap to code" so you
	 * would give the parent name as "Cheap to code", the parent table as
	 * "claims", the name of the field that will identy the ontology (rather
	 * misleadingly called "elementName" here) as "ontology", and the element
	 * type as ONTENTRY.
	 * 
	 * @param parentName
	 *            - the name of the parent element
	 * @param parentTable
	 *            - the type of table for the parent
	 * @param elementName
	 *            - the name of the field in the parent's table that contains
	 *            our element
	 * @param elementType
	 *            - the type of element we are trying to get
	...
	private String getElement(String parentName, String parentTable,
		...
		try {
			...
			findQuery = "SELECT name FROM "
					+ RationaleDBUtil.escapeTableName(RationaleDB
							.getTableName(elementType)) + " WHERE id = "
	...
	/**
	 * Delete a pattern from the database. Before deleting, must first delete
	 * all references to this pattern.
	 * 
	 * @param patternName
	 */
	public void removePattern(String patternName) {
		...
		try {
			...
			Vector<Integer> assocDecision = new Vector<Integer>(); // vector of
																	// all
																	// associated
																	// pattern-decisions

			query = "SELECT id from PATTERNDECISIONS where parent = "
					+ patternID;
			rs = stmt.executeQuery(query);
			while (rs.next()) {
			...
			while (decIterator.hasNext()) {
				query = "DELETE FROM PATTERN_DECISION where decisionID = "
						+ Integer.parseInt(decIterator.next().toString());
				stmt.execute(query);
			}
			query = "DELETE FROM PATTERNDECISIONS where parent = " + patternID;
			stmt.execute(query);
			query = "DELETE FROM PATTERN_ONTENTRIES where patternID = "
					+ patternID;
			stmt.execute(query);
	...
	/**
	 * Delete a tactic from database. Before deleting, must delete all
	 * references to the tactic first.
	 * 
	 * @param tacticName
	 */
	public void removeTactic(String tacticName) {
		...
		try {
			...
			stmt.execute(query);
			query = "DELETE FROM TACTIC_NEGONTENTRIES WHERE tactic_id = "
					+ tacticID;
			stmt.execute(query);
	...
	/**
	 * Create a vector of new OntEntries that are associated with the given
	 * pattern(name).
	 * 
	 * @param parentName
	 * @param isPositive
	 * @return
	 */
	public Vector<OntEntry> getPatternOntologies(String parentName,
			boolean isPositive) {
	...
	/**
	 * Get a list of alternatives GENERATED by generate pattern wizard. Only
	 * those patterns belong to the category will be added to the vector.
	 * 
	 * @param category
	 *            The pattern category of the alternative.
	 * @return A vector of alternatives generated from pattern generation
	 *         wizard.
	 */
	public Vector<Alternative> getAlternativesFromPattern(String category) {
		Vector<Alternative> toReturn = new Vector<Alternative>();
		Statement stmt = null;
		ResultSet rs = null;
		String findQuery = "";

		try {
			findQuery = "SELECT a.name FROM alternatives a, patterns p WHERE a.patternid >= 0 AND "
					+ "a.patternid = p.id AND p.type = '" + category + "'";
			stmt = conn.createStatement();
			rs = stmt.executeQuery(findQuery);
			while (rs.next()) {
				Alternative alt = new Alternative();
				alt.fromDatabase(rs.getString(1));
				toReturn.add(alt);
			}
		} catch (SQLException ex) {
			reportError(ex, "error in getAlternativesFromPattern", findQuery);
		} finally {
			releaseResources(stmt, rs);
		}
		return toReturn;
	...
	/**
	 * Create a vector of new Candidate Patterns that are associated with the
	 * given pattern(name)
	 * 
	 * @param parentName
	 * @return
	...
	public Vector<Pattern> getCandidatePatterns(String parentName) {
		...
		try {
			...
			findQuery = "SELECT patternID FROM pattern_decision WHERE decisionID = "
					+ parentDecision.getID() + " and parentType = 'Decision'";
			stmt = conn.createStatement();
			rs = stmt.executeQuery(findQuery);
			while (rs.next()) {
	...
	/**
	 * Create a vector of decisions for a given pattern(name) and its
	 * grandparent(name).
	 * 
	 * @param parentName
	 ...
	 * @return
	 */
	public Vector<PatternDecision> getPatternDecisions(String parentName,
		...
		try {
			...
			findQuery = "SELECT * FROM patterndecisions where parent = "
					+ patternID;
			rs = stmt.executeQuery(findQuery);

			while (rs.next()) {
				PatternDecision patternDecision = new PatternDecision();
				patternDecision.fromDatabase(RationaleDBUtil.decode(rs
						.getString("name")));
	 ...
	 */
	public Vector<String> getConstraints(String parentName,
		...
		try {
			...
			if (ptype == RationaleElementType.CONSTRAINT) {
				...
				findQuery = "SELECT child FROM "
						+ RationaleDBUtil
								.escapeTableName("ConstraintRelationships")
						+ " " + " WHERE parent = "
						+ new Integer(pid).toString();
				...
				findQuery = "SELECT constr FROM "
						+ RationaleDBUtil
								.escapeTableName("ConDecRelationships") + " "
			...
			while (ids.hasMoreElements()) {
				...
				rs = stmt.executeQuery(findQuery);
				if (rs.next()) {
					consts.addElement(RationaleDBUtil.decode(rs
							.getString("name")));
					rs.close();
				}
			}
	 ...
	 * 
	 * @param parentName
	 *            - the name of the parent alternative
	...
	public Vector<String> getAltConstRels(String parentName) {
		...
		try {
			...
			while (rs.next()) {
				relationships.addElement(RationaleDBUtil.decode(rs
						.getString("name")));
			}
			rs.close();
	...
	/**
	 * Gets a list of child design components given the name of a parent
	 * component
	 * 
	 * @param parentName
	...
	public Vector<String> getProducts(String parentName) {
		...
		try {
			...
			findQuery = "SELECT child FROM "
					+ RationaleDBUtil
							.escapeTableName("DesignComponentRelationships")
					+ " " + " WHERE parent = " + new Integer(pid).toString();
			...
			while (ids.hasMoreElements()) {
				...
				rs = stmt.executeQuery(findQuery);
				if (rs.next()) {
					products.addElement(RationaleDBUtil.decode(rs
							.getString("name")));
					rs.close();
				}
			}
	...
	public Vector<OntEntry> getOntologyElements(String parentName) {
		...
		try {
			...
			reportError(ex, "Error in getOntologyElements",
					"getOntologyElements");
		} finally {
	...
	public Vector<OntEntry> getOntologyDescendents(String subrootName) {
		Vector<OntEntry> onts = new Vector<OntEntry>();
		onts.addAll(getOntologyElements(subrootName));
		Vector<OntEntry> children = new Vector<OntEntry>();
		for (OntEntry ent : onts) {
			children.addAll(getOntologyDescendents(ent.getName()));
		}
		onts.addAll(children);
		return onts;
	}

	/**
	 * Helper method for recursion. Get all ontology at a particular level of
	 * the tree.
	 * 
	 * @param entries
	 * @param curLevel
	 * @param targetLevel
	 * @return null if entries is empty. Otherwise, returns a vector containing
	 *         all ontology elements at a particular level.
	 */
	private Vector<OntEntry> getOntologiesAtLevel(Vector<OntEntry> entries,
			int curLevel, int targetLevel) {
		if (entries == null)
			return null;
		if (curLevel < targetLevel) {
			Vector<OntEntry> children = new Vector<OntEntry>();
			for (OntEntry ent : entries) {
				children.addAll(getOntologyElements(ent.getName()));
			}
			curLevel++;
			return getOntologiesAtLevel(children, curLevel, targetLevel);
		} else {
			Vector<OntEntry> toReturn = new Vector<OntEntry>();
			for (OntEntry ent : entries) {
				toReturn.addAll(getOntologyElements(ent.getName()));
			}
			return toReturn;
		}
	...
	/**
	 * Get a vector of all ontologies at a level under the subroot. NOTE: the
	 * level of the subroot is 0.
	 * 
	 * @param subroot
	 * @param targetLevel
	 * @return null if subroot == null. Otherwise, return a vector containing
	 *         all elements that are targetLevel down to the tree from subroot.
	 */
	public Vector<OntEntry> getOntologiesAtLevel(OntEntry subroot,
			int targetLevel) {
		if (subroot == null)
			return null;
		Vector<OntEntry> entries = new Vector<OntEntry>();
		entries.add(subroot);
		return getOntologiesAtLevel(entries, 1, targetLevel);
	}

	/**
	 * Find the relative level of the descendant to the parent.
	 * 
	 * @param parent
	 * @param desc
	 *            descendant
	 * @return -1 if desc is not a descendant of parent. Otherwise, returns the
	 *         relative sublevel of descendant to the parent.
	 */
	public int findRelativeOntLevel(OntEntry parent, OntEntry desc) {
		Vector<OntEntry> cur = new Vector<OntEntry>();
		cur.add(parent);
		if (parent.equals(desc))
			return 0;
		int level = 1;
		Vector<OntEntry> next = getOntologiesAtLevel(parent, level);
		while (!cur.equals(next)) {
			cur = next;
			if (cur.contains(desc))
				return level;
			level++;
			next = getOntologiesAtLevel(parent, level);
		}
		return -1;
	}

	/**
	 * Given the name of a element of argument ontology, retrieve all of its
	 * leaves.
	 * 
	 * @param parentName
	 *            the name to retrieve leaves from.
	 * @return
	 */
	...
	/**
	 * Return a list of child element entries given the parent for treeview
	 * display
	 * 
	 * @param parentName
	 *            - the name of the parent
	 ...
	 * @return a list of child elements - the actual elements, not just names
	 */
	public Vector<String> getElements_TreeView(String parentName,
			RationaleElementType type, boolean firstTime) {
		...
		try {
			...
			if (firstTime) {
				if (type == RationaleElementType.REQUIREMENT)
					...
					selectStr = "Select * from (SELECT a.id as childID,a.ptype,a.name as childName,a.parent as parentID,b.name as parentName FROM (alternatives as a) left join (alternatives as b) on (a.parent=b.id)) as t where t.ptype="
							+ "'Decision'";
				...
				if (type == RationaleElementType.REQUIREMENT)
					selectStr = "Select * from (SELECT a.id as childID,a.name as childName,a.parent as parentID,b.name as parentName FROM (requirements as a) left join (requirements as b) on (a.parent=b.id)) as t where t.parentName="
							+ "'" + parentName + "'";
				else if (type == RationaleElementType.DECISION)
					selectStr = "Select * from (SELECT a.id as childID,a.name as childName,a.parent as parentID,b.name as parentName FROM (decisions as a) left join (decisions as b) on (a.parent=b.id)) as t where t.parentName="
							+ "'" + parentName + "'";
				else if (type == RationaleElementType.ALTERNATIVE)
					selectStr = "Select * from (SELECT a.id as childID,a.name as childName,a.parent as parentID,b.name as parentName FROM (alternatives as a) left join (alternatives as b) on (a.parent=b.id)) as t where t.parentName="
							+ "'" + parentName + "'";
				else if (type == RationaleElementType.ARGUMENT)
					selectStr = "Select * from (SELECT a.id as childID,a.name as childName,a.parent as parentID,b.name as parentName FROM (arguments as a) left join (arguments as b) on (a.parent=b.id)) as t where t.parentName="
							+ "'" + parentName + "'";
			}
			...
			reportError(ex, "Error in getElements_TreeView method",
					"getOntologyElements");
		} finally {
			releaseResources(null, rs);
		}
		...
		try {
			...
			while (ids.hasMoreElements()) {
				...
				if (rs.next()) {
					...
					entry.fromDatabase(RationaleDBUtil.decode(rs
							.getString("name")));
	 ...
	 * 
	 * @param type
	 *            - the type of element we are looking for
	...
	public Vector<String> getNameList(RationaleElementType type) {
		...
		try {
			...
			while (rs.next()) {
				ourElements.addElement(RationaleDBUtil.decode(rs
						.getString("name")));
			}
	...
	/**
	 * This method provides better running time than getPatternByCategory. Used
	 * for generate candidate patterns.
	 * 
	 * @param categoryID
	 * @return
	 */
	public ArrayList<Pattern> getPatternByCategoryID(int categoryID) {
		...
		try {
			...
			ResultSet rs = null;
			stmt = conn.createStatement();
			query = "select patternID from pattern_problemcategory where problemcategoryID = "
					+ categoryID;
			rs = stmt.executeQuery(query);
			while (rs.next()) {
	...
	public ArrayList<Pattern> getPatternByCategory(String category) {
		...
		try {
			...
			ResultSet rs = null;
			stmt = conn.createStatement();
			query = "SELECT id FROM patternproblemcategories WHERE problemcategory = '"
					+ category + "'";
			rs = stmt.executeQuery(query);
	 ...
	 * 
	 * @param patternID
	 * @return -1 if it is not associated, otherwise it returns the ID of the
	 *         problem category.
	 */
	public int getCategoryByPattern(int patternID) {
		...
		try {
			stmt = conn.createStatement();
			String query = "SELECT problemcategoryID from PATTERN_PROBLEMCATEGORY WHERE patternID = "
					+ patternID + "";
			rs = stmt.executeQuery(query);
			if (rs.next()) {
	...
	public HashMap<String, Integer> getCategories(String patternType) {
		...
		// Wang uses upper case for archiecture name, so we should 'convert' it
		// first...
		/*
		 * char upperCaseBeginning = patternType.toUpperCase().charAt(0); String
		 * correctedType = upperCaseBeginning + patternType.substring(1);
		...
		try {
			stmt = conn.createStatement();
			String query = "SELECT id, problemcategory FROM patternproblemcategories  WHERE patterntype = '"
					+ patternType + "'";
			rs = stmt.executeQuery(query);
			while (rs.next()) {
	...
	/**
	 * Given a pattern type, return all patterns matches the type. The type is
	 * "archiecture", "idiom", or "design"
	 * 
	 * @param patternType
	...
	private Vector<TreeParent> getPatterns(String patternType) {
		...
		try {
			stmt = conn.createStatement();
			query = "SELECT name FROM PATTERNS WHERE type = '" + patternType
					+ "'";
			rs = stmt.executeQuery(query);
			// while(rs.next()){
	...
	/**
	 * Given positive quality attribute, get tactics helps the quality
	 * attribute.
	 * 
	 * @param category
	 *            positive quality attribute. If null, then get all tactics from
	 *            DB.
	...
	public Vector<TreeParent> getTactics(String category) {
		...
		try {
			...
			while (rs.next()) {
				TreeParent element = new TreeParent(RationaleDBUtil.decode(rs
						.getString("name")), RationaleElementType.TACTIC);
	...
	public ArrayList<Pattern> getPatternsByType(String type) {
		...
		try {
			stmt = conn.createStatement();
			query = "SELECT name FROM PATTERNS WHERE type = '" + type + "'";
			rs = stmt.executeQuery(query);
			while (rs.next()) {
				Pattern pattern = new Pattern();
				pattern.fromDatabase((rs.getString("name")));
	...
	/**
	 * Get a list of names of all elements in the database that have a
	 * particular rationale element type and where the name contains a search
	 * string
	 * 
	 * @param type
	 *            - the type of element we are looking for
	 * @param sstring
	 *            - the substring we want to find in the name
	 * @return - the list of names
	 */
	public Vector<String> getNameList(RationaleElementType type, String sstring) {
		...
		try {
			...
			findQuery = "SELECT name FROM "
					+ RationaleDBUtil.escapeTableName(tableName) + " "
					+ additional + "WHERE upper(name) LIKE" + " upper('%"
					+ sstring + "%')" + " ORDER BY name ASC";
			System.out.println(findQuery);
			rs = stmt.executeQuery(findQuery);
			...
			reportError(ex, "Error in getNameList with Search String",
					findQuery);
		} finally {
			releaseResources(stmt, rs);
		}
	...
	/**
	 * Get a list of all "potential new parents" for a particular type of
	 * candidate rationale element
	 * 
	 * @param type
	 *            - the type of element whose new parent we are looking for
	 * @return - the list of names
	 */
	...
	public Vector<String> getRationaleElementList(RationaleElementType type) {
		...
		try {
			...
			/*
			 * else if (type == RationaleElementType.ARGUMENT) { findQuery =
			 * "(SELECT name FROM requirements) union (select name FROM alternatives)"
			 * ; }
			 */
	...
	/**
	 * Get a list of all "potential new parents" for Argument type of rationale
	 * element
	 * 
	 * @param type
	 *            - the type of element whose new parent we are looking for
	 * @param ptype
	 *            - parent's the type of element
	 * @return - the list of names
	 */
	public Vector<String> getRationaleElementList(RationaleElementType type,
			RationaleElementType ptype) {
		...
		try {
			...
			if (type == RationaleElementType.ARGUMENT
					&& ptype == RationaleElementType.REQUIREMENT) {
				findQuery = "SELECT name FROM requirements";
			}
			if (type == RationaleElementType.ARGUMENT
					&& ptype == RationaleElementType.ALTERNATIVE) {
				findQuery = "select name FROM alternatives";
			}
			if (type == RationaleElementType.DECISION
	...
	/**
	 * Returns a list of TreeParent items (to put in the tree from the
	 * RationaleExplorer) from the database given the parent name, the element
	 * type, and the parent type.
	 * 
	 * @param parentName
	 *            - parent name
	 * @param elementType
	 *            - type of element we are looking for (table name)
	 * @param parentType
	 *            - parents element type
	 * @return a list of tree elements
	 */
	private Vector<TreeParent> getTreeElements(String parentName,
			String elementType, RationaleElementType parentType) {
		...
		try {
			...
			if (parentType == null) {
				...
				findQuery = "SELECT id FROM "
						+ RationaleDBUtil
								.escapeTableName(getTableName(parentType))
						+ "  WHERE name= '"
						+ RationaleDBUtil.escape(parentName) + "'";
				rs = stmt.executeQuery(findQuery);
				if (rs.next()) {
				...
				/*
				 * already have parentType==null in the front. This can't
				 * happen. if (parentType == null) { findQuery =
				 * "SELECT name FROM " +
				 * RationaleDBUtil.escapeTableName(elementType) + " " +
				 * " WHERE parent = " + new Integer(pid).toString() +
				 * " ORDER BY name ASC"; } else {
				 */
				findQuery = "SELECT name FROM "
						+ RationaleDBUtil.escapeTableName(elementType) + " "
						+ " WHERE parent = " + new Integer(pid).toString()
			...
			if ((type == RationaleElementType.REQUIREMENT)
				...
				while (stats.hasMoreElements()) {
					...
					RationaleErrorLevel ourStatus = getActiveStatus(
	...
	/**
	 * Returns a list of CandidateTreeParent items (to put in the tree from the
	 * RationaleExplorer) from the database given the element type
	 * 
	 * @param elementType
	 *            - type of element we are looking for
	 * @return a list of tree elements
	 */
	public Vector<CandidateTreeParent> getCandidateTreeElements(
			RationaleElementType etype) {
	 ...
	 *            - the name of the parent alternative
	 * @param parentType
	 *            - it is not clear why this is needed! Probably a cut-and-paste
	 *            artifact...
	...
	/**
	 * Find out what error level an element has (error, warning, information).
	 * If there is more than one error, return the most severe level
	 * 
	 * @param name
	 *            - the name of the rationale element
	 * @param elementType
	 *            - the type of the rationale element
	...
	public RationaleErrorLevel getActiveStatus(String name,
		...
		try {
			...
			findQuery = "SELECT S.type from "
					...
					+ " S, "
					+ RationaleDBUtil
							.escapeTableName(getTableName(elementType)) + " P"
					+ " where P.name = '" + name + "' and "
					+ "P.id = S.parent " + "and S.ptype = '"
	...
	/*
	 * public RationaleErrorLevel getStatus(String name, RationaleElementType
	 * elementType) { String findQuery = ""; RationaleErrorLevel worst;
	 * Statement stmt = null; ResultSet rs = null; worst =
	 * RationaleErrorLevel.INFORMATION; //just in case name =
	 * RationaleDB.escape(name); try { stmt = conn.createStatement(); ;
	 * findQuery = "SELECT S.type from status S, " + getTableName(elementType) +
	 * " P" + " where P.name = '" + name + "' and " + "P.id = S.parent " +
	 * "and S.ptype = '" + elementType.toString() + "'"; //***
	 * System.out.println(findQuery); rs = stmt.executeQuery(findQuery);
	 * 
	 * while (rs.next()) { // System.out.println("found element"); //
	 * System.out.println("type = " + rs.getString("type")); RationaleErrorLevel
	 * newStat = RationaleErrorLevel.fromString(rs .getString("type")); if
	 * (worst == RationaleErrorLevel.INFORMATION) { worst = newStat; } else if
	 * (worst == RationaleErrorLevel.WARNING) { if (newStat ==
	 * RationaleErrorLevel.ERROR) { worst = newStat; } } }
	 * 
	 * } catch (SQLException ex) { reportError(ex, "SQLException, get status: "
	 * + ex.getMessage(), findQuery); } finally { releaseResources(stmt, rs); }
	 * return worst;
	 * 
	 * }
	 */
	...
	/**
	 * Find the rationale error level given an element ID and type. Seems a bit
	 * redundant with ActiveStatus which uses the name and type...
	 * 
	 * @param pid
	 *            - the element ID
	 * @param elementType
	...
	public RationaleErrorLevel getStatusLevel(int pid,
		...
		try {
			...
			findQuery = "SELECT S.type from "
					...
					+ " S, "
					+ RationaleDBUtil
							.escapeTableName(getTableName(elementType))
					+ " P where " + "S.parent = " + new Integer(pid).toString()
					+ " and S.ptype = '" + elementType.toString()
					+ "' and S.override = 'No'";
			// *** System.out.println(findQuery);
			rs = stmt.executeQuery(findQuery);
	...
	public Vector<RationaleStatus> getStatus() {
		...
		try {
			...
			while (rs.next()) {
				RationaleStatus rstat = new RationaleStatus(
						RationaleErrorLevel.fromString(rs.getString("type")),
						RationaleDBUtil.decode(rs.getString("description")),
	 ...
	 * they do not want to see it appear on the list of errors and warnings. Of
	 * course, this can easily be reversed later.
	 * 
	 * @param descrip
	 *            - the error/warning description
	 * @param parent
	 *            - the element the error/warning is about
	 * @param ptype
	 *            - the type of element the error/warning is about
	 */
	public void overrideStatus(String descrip, int parent, String ptype) {
		...
		try {
			...
			findQuery = "SELECT status  FROM "
					+ RationaleDBUtil.escapeTableName("status")
					+ " WHERE parent = " + new Integer(parent).toString()
					+ " and ptype = '" + ptype + "'" + " and description = '"
			...
			if (rs.next()) {
				...
				updateQuery = "UPDATE "
						+ RationaleDBUtil.escapeTableName("Status") + " "
						+ "SET override = 'Yes' " + "WHERE status = '" + stat
						+ "' AND parent = " + new Integer(parent).toString()
						+ " AND ptype = '" + ptype + "'";
	...
	public Vector<RationaleStatus> getStatus(int ID, RationaleElementType type) {
		...
		try {
			...
			findQuery = "SELECT *  FROM "
					+ RationaleDBUtil.escapeTableName("status") + " "
					+ " WHERE parent = " + new Integer(ID).toString()
					+ " and ptype = '" + type.toString() + "'";
			// *** System.out.println(findQuery);
			rs = stmt.executeQuery(findQuery);
			...
			while (rs.next()) {
				RationaleStatus rstat = new RationaleStatus(
						...
						RationaleElementType.fromString(rs.getString("ptype")),
						rs.getDate("date"), rs.getInt("parent"),
						RationaleStatusType.fromString(RationaleDBUtil
								.decode(rs.getString("status"))));
				if (rs.getString("override").compareTo("Yes") == 0) {
					rstat.setOverride(true);
				}
				statusList.add(rstat);
	...
	/**
	 * Add a new status elements to the database. This is done because SEURAT
	 * detected problems with the rationale.
	 * 
	 * @param newStatus
	 *            - the status element
	 */
	...
	/**
	 * Remove status elements from the database (presumably because the problem
	 * with the rationale has been fixed).
	 * 
	 * @param oldStatus
	 *            - the status element to remove
	 */
	...
	/**
	 * Associate an alternative with a resource. In Eclipse, a resource can be a
	 * file, attribute, or method. This information is actually maintained using
	 * markers (bookmarks and special markers to put the rat icon on the file)
	 * but we also store it in the database so we can share it between machines.
	 * The database schema also includes the line number, but this is useless in
	 * helping us restore the markers between sessions, so the method no longer
	 * uses it.
	 * 
	 * @param alternative
	 *            - the ID of the alternative being associated
	 * @param artifact
	 *            - the Eclipse format artifact identifier
	 * @param resource
	 *            - the file name for the resource - this is what gets displayed
	 *            in the bookmark
	 * @param name
	 *            - the name of the method or attribute we are associating with
	 * @param message
	 *            - the name of the alternative that is displayed
	 */
	public void associateAlternative(String alternative, String artifact,
		...
		try {
			...
			if (rs.next()) {
				int altID = rs.getInt("id");
				// TODO get alternative object here so that it can be used in an
				// update event
	...
	/**
	 * Given the Eclipse format resource, delete all associations to that
	 * resource from the associations table. This will remove all the
	 * associations to a file.
	 * 
	 * @param resource
	...
	/**
	 * Removes associations of a specific alternative to a specific file at a
	 * specific line number. This is very brittle - if the line number comes
	 * from a bookmark and the file has been edited it will not match the
	 * database!
	 * 
	 * @param resource
	 *            - the file name where the association is
	 * @param message
	 *            - the name of the alternative it was associated with
	 * @param lineNumber
	...
	/**
	 * Delete the candidate pattern associated with a pattern decision in
	 * Pattern Library
	 * 
	 * @param String
	 *            - the name of the pattern, String - the name of the parent
	 *            decision
	 */
	public void removeCandidatePattern(String patternName, String decisionName) {
		...
		try {
			...
			removeCanPattern = "Delete from pattern_decision where parentType = 'Decision' and patternID = "
					+ p.getID() + " and decisionID = " + pd.getID();
	 ...
	 * @param direction
	 */
	public void removePatternOnt(String ontName, String patternName,
			String direction) {
		Statement stmt = null;
		String removePOnt = "";
		try {
			...
			removePOnt = "Delete from pattern_ontentries where ontID = "
					+ oe.getID() + " and patternID = " + p.getID()
	 ...
	 * @param patternNames
	 */
	public void saveCandidatePatterns(String decisionName,
		...
		try {
			...
			for (int k = 0; k < patternNames.size(); k++) {
				...
				insertCanPattern = "INSERT INTO PATTERN_DECISION (patternID, decisionID, parentType) VALUES ("
						+ p.getID() + "," + pd.getID() + ", 'Decision')";
	...
	/**
	 * Remove an element from the database. This includes clearing out the
	 * history but doesn't clear out the status.
	 * 
	 * @param ele
	 *            - the rationale element to remove.
	 */
	public void deleteRationaleElement(RationaleElement ele) {
		...
		try {
			...
			deleteCmd = "DELETE FROM "
					+ RationaleDBUtil.escapeTableName(RationaleDB
							.getTableName(ele.getElementType()))
	...
	/**
	 * Remove a rationale candidate from the database. This can't use the
	 * generic deleteRationaleElement because the type doesn't signify the table
	 * name.
	 * 
	 * @param ele
	 *            - the rationale element to remove.
	...
	public int countArgReferences(RationaleElement ele) {
		...
		try {
			...
			findCmd = "Select id FROM "
					+ RationaleDBUtil.escapeTableName("arguments") + " "
					+ " where " + argField + " = " + new Integer(id).toString();
	...
	public Vector<Claim> getOverridenClaims() {
		...
		try {
			...
			findQuery = "SELECT *  FROM "
					+ RationaleDBUtil.escapeTableName("claims") + " "
					+ " WHERE importance <> '" + Importance.DEFAULT.toString()
					+ "'";
			// *** System.out.println(findQuery);
			rs = stmt.executeQuery(findQuery);
	...
	/**
	 * Release our database resources. This is done over and over and over again
	 * so we have this nifty utility method to cut down on duplicated code.
	 * 
	 * @param stmt
	...
	/**
	 * Release our database resources. This is done over and over and over again
	 * so we have this nifty utility method to cut down on duplicated code. We
	...
	/**
	 * Report an error that occured when trying to do something with the
	 * database.
	 * 
	 * @param ex
	 *            - the SQL exception that has occured
	 * @param location
	 *            - the part of SEURAT that ran into trouble (typically a method
	 *            name)
	 * @param message
	...
	public static String getOntName() {
		try {
			URL ratDBCreateURL = FileLocator.resolve(new URL(ratDBPath,
					"ratDBCreate.xml"));
	...
	private static void readXMLData(Document ratDoc) {
		...
		while (ratNext != null) {
			...
			if (nextName.compareTo("DR:argOntology") == 0) {
				...
				// nothing to do here. it is already in the database and it's a
				// top
				// level item
	...
	/**
	 * Exports the argument ontology to XML by calling the appropriate utility
	 * function and passing it the name of the file we are exporting to.
	 * 
	 * @return a boolean indicating success or failure which can be used to
	...
	/**
	 * Exports the whole db to an XML file. (Currently, we're only exporting
	 * pattern library and the ontology) This is done by calling the utlity
	 * function in RationaleDBUtil.
	 * 
	 * @param path
	...
	public String getRef(Pattern p) {
		...
		while (keyIterator.hasNext()) {
			...
			if (p.equals(xmlRefs[PATTERN_XML_INDEX].get(current)))
				return current;
		}
		return null;
	}
	...
	public String getRef(PatternDecision d) {
		Iterator keyIterator = xmlRefs[PATTERNDECISION_XML_INDEX].keySet()
				.iterator();
		while (keyIterator.hasNext()) {
			String current = (String) keyIterator.next();
			if (d.equals(xmlRefs[PATTERNDECISION_XML_INDEX].get(current)))
				return current;
		}
		return null;
	}
	...
	public String getRef(String id, String problem, String type) {
		Iterator keyIterator = xmlRefs[PATTERNPROBLEMCATEGORY_XML_INDEX]
				.keySet().iterator();
		while (keyIterator.hasNext()) {
			String current = (String) keyIterator.next();
			String[] curValue = (String[]) xmlRefs[PATTERNPROBLEMCATEGORY_XML_INDEX]
					.get(current);
	...
	/**
	 * this is the version of getRef that returns a rationale element this is
	 * used when reading in rationale from the XML, in this case, storing ids is
	 * not sufficient
	 */
	 ...
	 * Note that at this point, the database may violate referential integrity
	 * constraints.
	 * 
	 * @param pattern
	 */
	public void addPatternFromXML(Pattern pattern) {
		...
		try {
			...
			PreparedStatement ps = conn
					.prepareStatement("INSERT INTO PATTERNS values ("
							+ "?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			...
			// connection with sub-decision must be done in utility after
			// decision has been created...
		} catch (java.sql.SQLIntegrityConstraintViolationException e) {
			System.out.println("WARNING: Skipping duplicated entry "
					+ pattern.getName());
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	/**
	 * Given a pattern decision imported from xml, add it to the database. Note
	 * that the parent value must be updated later.
	 * 
	 * @param pd
	 */
	public void addPatternDecisionFromXML(PatternDecision pd) {
		try {
			...
			String expr = "INSERT INTO PATTERNDECISIONS (id, name, description, type, status, phase, ptype, subdecreq)"
					...
					+ pd.getID()
					+ ", '"
					+ RationaleDBUtil.escape(pd.getName())
					+ "', '"
					+ RationaleDBUtil.escape(pd.getDescription())
					+ "', '"
					+ pd.getType().toString()
					+ "', '"
					+ pd.getStatus().toString()
					+ "', '"
					+ pd.getPhase().toString() + "', 'Pattern', 'No')";
			stmt.execute(expr);
			...
			while (candidatePID.hasNext()) {
				...
				expr = "INSERT INTO PATTERN_DECISION values (" + cpid + ", "
						+ pd.getID() + ", 'Decision')";
				stmt.execute(expr);
			}
			stmt.close();
	...
	public void addTacticFromXML(Tactic t) {
		try {
			...
			String expr = "INSERT INTO TACTICS (id, name, quality, description, time_beh) "
					...
					+ t.getID()
					+ ", '"
					+ RationaleDBUtil.escape(t.getName())
					+ "', "
					+ t.getCategory().getID()
					+ ", '"
					+ RationaleDBUtil.escape(t.getDescription())
					+ "', "
					+ t.getTime_behavior() + ")";
			stmt.execute(expr);
			...
			while (effectsI.hasNext()) {
				int effectID = RationaleDB
						.findAvailableID("TACTIC_NEGONTENTRIES");
				OntEntry cur = effectsI.next();
				expr = "INSERT INTO TACTIC_NEGONTENTRIES "
						+ " (id, tactic_id, ont_id) VALUES (" + effectID + ", "
						+ t.getID() + ", " + cur.getID() + ")";
				stmt.execute(expr);
			}
			...
			if (e.getMessage().contains("duplicate"))
				...
				System.out.println("WARNING: Skipping invalid entry "
						+ t.getName());
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public void addTacticPatternFromXML(TacticPattern tp) {
		try {
			...
			String expr = "INSERT INTO TACTIC_PATTERN "
					...
					+ "values (" + tp.getID() + ", " + tp.getTacticID() + ", "
					+ tp.getPatternID() + ", " + tp.getStruct_change() + ", "
					+ tp.getNumChanges() + ", " + tp.getBeh_change() + ", "
					+ tp.getOverallScore() + ", '"
					+ RationaleDBUtil.escape(tp.getDescription()) + "')";
			stmt.execute(expr);
		} catch (java.sql.SQLIntegrityConstraintViolationException e) {
	...
	/**
	 * Assume pattern and patterndecisions have been imported. Utility can call
	 * this method to associate between pattern and child pattern-decisions.
	 * 
	 * @param patternID
	 * @param pdID
	 */
	public void assocPatternAndDecisionFromXML(int patternID, int pdID) {
		try {
			Statement stmt = conn.createStatement();
			String expr = "UPDATE PATTERNDECISIONS set parent = " + patternID
					+ " WHERE id = " + pdID;
			stmt.execute(expr);
		} catch (SQLException e) {
			e.printStackTrace();
	...
	/**
	 * This method is used for XML import of pattern problem category. Given the
	 * database id, category, and type of the problem category, add it to the
	 * database.
	 * 
	 * @param id
	 *            The parsed id of the reference id in XML
	 * @param category
	 * @param type
	 */
	public void addProblemCategory(int id, String category, String type) {
		try {
			...
			String expr = "INSERT INTO PATTERNPROBLEMCATEGORIES values ("
					+ new Integer(id).toString() + ", '" + category + "', '"
					+ type + "')";
			stmt.execute(expr);
			stmt.close();
		} catch (SQLIntegrityConstraintViolationException e) {
			System.out
					.println("WARNING: Problem category already exists. Skipping.");
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
	...
	/**
	 * This method returns all database info about problem category. Used for
	 * XML export String[0] represents the id of each problem category String[1]
	 * represents the problem category names String[2] represents the type of
	...
	public Vector<String[]> getProblemCategoryData() {
		...
		try {
			...
			String query = "SELECT *" + "FROM PATTERNPROBLEMCATEGORIES";
	...
	/**
	 * This method returns all database info about patterns. Used for XML
	 * export.
	 * 
	 * @return
	 */
	...
	/**
	 * This method returns all database info about tactics. Used for XML export.
	 * 
	 * @return
	 */
	 ...
	 * are read from the file and stored into vectors. Then, this method is
	 * called to put the elements actually INTO the database.
	 * 
	...
	/**
	 * This method contains the hard-coded insertion of patterns. Used when XML
	 * is not there.
	 * 
	 * @param ps
	 */
	private void importPatterns(PreparedStatement ps) {
		try {
			...
			ps.setBytes(
					3,
					(new String(
							"The system is organized into three primary layers: Presentation, Domain, and Data Source."))
							.getBytes());
			ps.setBytes(
					4,
					(new String(
			...
			ps.setBytes(
					3,
					(new String(
							"The Blackboard architectural pattern is useful for problems for which no deterministic solution strategies are known. In Blackboard several specialized subsystem assemble their knowledge to build a possibly partial or approximate solution."))
							.getBytes());
			ps.setBytes(4, (new String("")).getBytes());
			ps.setBytes(5, (new String("")).getBytes());
			ps.setBytes(6, (new String("")).getBytes());
			...
			ps.setBytes(
					3,
					(new String(
							"Provide a unified interface to a set of interfaces in a subsystem. Fa?ade defines a higher-level interface that makes the subsystem easier to user."))
							.getBytes());
			ps.setBytes(4, (new String("")).getBytes());
			ps.setBytes(5, (new String("")).getBytes());
			ps.setBytes(6, (new String("'")).getBytes());
			...
			ps.setBytes(8, (new String("")).getBytes());
			ps.setString(9,
					"http://vico.org/pages/PatronsDisseny/Pattern%20Bridge/");
			ps.setInt(10, RationaleDBCreate.pushCurrentID());
			ps.executeUpdate();
			...
			ps.setBytes(
					3,
					(new String(
							"This idiom makes memory management of dynamically-allocated shared objects in C++ easier. It introduces a reference counter to a body class that is updated by handle objects. Clients access body class objects only through handles via the overloaded operator ->().."))
							.getBytes());
			ps.setBytes(4, (new String("")).getBytes());
			ps.setBytes(5, (new String("")).getBytes());
			ps.setBytes(6, (new String("'")).getBytes());
			...
			ps.setBytes(8, (new String("")).getBytes());
			ps.setString(9,
					"http://vico.org/pages/PatronsDisseny/Pattern%20Singleton/");
			ps.setInt(10, RationaleDBCreate.pushCurrentID());
			ps.executeUpdate();
	...
	public static Vector<XFeatureMapping> getToplevelMappings() {
		...
		try {
			...
			reportError(ex, "Error in getting top level xfeature elements",
					findQuery);
		} finally {
			releaseResources(stmt, rs);
	...
	public static Vector<XFeatureMapping> getDependentMappings(int parent) {
		...
		try {
			...
			reportError(ex, "Error in getting dependent xfeature elements",
					findQuery);
		} finally {
			releaseResources(stmt, rs);
	...
	/**
	 * It seems the XML Import/Export has broken the ID auto_increment. I need
	 * this for every insertion of all entities. This method returns the next
	 * available ID after the max of ID's given dbName.
	 * 
	 * @param tableName
	 *            The table name to find the max ID.
	 */
	 ...
	 * @return empty vector if the no such pattern exist. Otherwise return as
	 *         specified.
	 */
	public Vector<PatternParticipant> getParticipantsFromPatternName(
			String patternName) {
		Pattern pattern = new Pattern();
	 ...
	 * 
	 * @param patternID
	 *            -1 if to get all participants, otherwise provide a valid
	 *            patternID.
