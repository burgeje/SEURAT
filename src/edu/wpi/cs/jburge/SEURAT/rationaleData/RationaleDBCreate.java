...
{	
	/**
	 * This variable is used to keep track of batch insert ID
	 */
	...
	private static String ontEntryInsert(String pString)
	{
		...
		//Now we have XML, there's no need.
		//String newPString = pString.substring(pString.indexOf(",") + 1);
		return "INSERT INTO ontentries (id, name, description, importance) " +
		"values (" + pString + ")";
	}
	...
	/**
	 * This method is called every time a hard-coded batch is completed its insertion.
	 */
	...
	/**
	 * Push the currentID by one, and then return the old value.
	 * @return
	 */
	public static int pushCurrentID(){
		int id = currentID;
		currentID++;
		return id;
	}
	...
	/**
	 * Hard coded pattern-decision insert. It uses currentID to batch insert.
	 * @param pdString
	 * @return
	 */
	private static String patternDecisionInsertH(String pdString){

		return "INSERT INTO patterndecisions (id, name, description, type, status, phase, subdecreq, ptype, parent) values (" + pushCurrentID() + ", " + pdString + ")";
	}
	...
	public static String patternProblemCategoriesInsert(String poString){
		...
		try{
			...
			return "INSERT INTO patternproblemcategories (id, problemcategory, patterntype) values (" + id + "," + poString + ")";
		} catch (Exception e){
	...
	/**
	 * This allows hard-coded batch insertion
	 * @param poString
	 * @return
	 */
	private static String patternProblemCategoriesInsertH(String poString){
		return "INSERT INTO patternproblemcategories (id, problemcategory, patterntype) values (" + pushCurrentID() + ", " + poString + ")";
	}
	...
	{
		String toReturn[] = {
				patternInsert("'Three-layer', ÔArchitectureÕ," + "'The system is organized into three primary layers: Presentation, Domain, and Data Source.'" +
				...
				patternInsert("'Layers', ÔArchitectureÕ,'The Layers architectural pattern helps to structure applications that can be decomposed into groups of subtasks in which each group of subtasks is at a particular level of abstraction.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Layers/'"),
				patternInsert("'Pipes and Filters', ÔArchitectureÕ,'The Pipes and Filters architectural pattern provides a structure for systems that process a stream of data. Each processing step is encapsulated in a filter component. Data is passed through pipes between adjacent filters. Recombining filters allows you to build families of related systems.'" +
				",'','','','','','http://msdn.microsoft.com/en-us/library/ms978599.aspx'"),
				patternInsert("'Blackboard', ÔArchitectureÕ,'The Blackboard architectural pattern is useful for problems for which no deterministic solution strategies are known. In Blackboard several specialized subsystem assemble their knowledge to build a possibly partial or approximate solution.'" +
				",'','','','','','http://www.vico.org/pages/PatronsDisseny/Pattern%20Blackboard/'"),
				patternInsert("'Model-View-Controller', ÔArchitectureÕ,'The MVC architectural pattern divides an interactive application into three components. The model contains the core functionality and data. Views display information to the user. Controllers handle user input. Views and Controllers together comprise the user interface. A change-propagation mechanism ensures consistency between the user interface and the model.'" +
				",'','','','','','http://msdn.microsoft.com/en-us/library/ms978748.aspx'"),
				patternInsert("'Broker', ÔArchitectureÕ,'The Broker architectural pattern can be used to structure distributed software systems with decoupled components that interact by remote service invocations. A broker component is responsible for coordinating communication, such as forwarding requests, as well as for transmitting results and exceptions.'" +
				",'','','','','','http://msdn.microsoft.com/en-us/library/ms978706.aspx'"),
				patternInsert("'Presentation-Abstraction-Control', ÔArchitectureÕ,'The Presentation-Abstraction-Control architectural pattern (PAC) defines a structure for interactive software systems in the form of a hierarchy of cooperating agents. Every agent is responsible for a specific aspect of the applications functionality and consists of three components: presentation, abstraction, and control. This subdivision separates the human-computer interaction aspects of the agent from its functional core and its communication with other agents.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Presentation%20Abstra/'"),
				patternInsert("'Microkernel', ÔArchitectureÕ,'The Microkernel architectural pattern applies to software systems that must be able to adapt to changing system requirements. It separates a minimal functional core from extended functionality and customer-specific parts. '" +
				",'','','','','','http://www.vico.org/pages/PatronsDisseny/Pattern%20MicroKernel/'"),
				patternInsert("'Reflection', ÔArchitectureÕ,'The Reflection architectural pattern provides a mechanism for changing structure and behavior of software systems dynamically. It supports the modification of fundamental aspects, such as type structures and function call mechanisms. In this pattern, an application is split into two parts. A meta level provides information about selected system properties and makes the software self-aware. A base level includes the application logic. Its implementation builds on the meta level. Changes to information kept in the meta level affect subsequent base-level behavior.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Reflection/'"),
				patternInsert("'Whole-Part', ÔDesignÕ,'The Whole-Part design pattern helps with the aggregation of components that together form a semantic unit. An aggregate component, the Whole, encapsulates its constituent components, the Parts, organizes their collaboration, and provides a common interface to its functionality. Direct access to the Parts is not possible.'" +
				",'','','','','','http://www.vico.org/pages/PatronsDisseny/Pattern%20Whole%20Part/index.html'"),
				patternInsert("'Master-Slave', ÔDesignÕ,'The Master-Slave design pattern supports fault tolerance, parallel computation and computational accuracy. A master component distributes work to identical slave components and computes a final result from the results these slaves return.'" +
				",'','','','','','http://www.vico.org/pages/PatronsDisseny/Pattern%20Master%20Slave/'"),
				patternInsert("'Proxy', ÔDesignÕ,'Provide a surrogate or placeholder for another object to control access to it.'" +
				",'','','','','','http://www.vico.org/pages/PatronsDisseny/Pattern%20Broker/'"),
				patternInsert("'Command Processor', ÔDesignÕ,'The Command Processor design pattern separates the request for a service from its execution. A command processor component manages requests as separate objects, schedules their execution, and provides additional services such as the storing of request objects for later undo.'" +
				",'','','','','','http://www.vico.org/pages/PatronsDisseny/Pattern%20Command%20Processor/index.html'"),
				patternInsert("'View Handler', ÔDesignÕ,'The View Handler design pattern helps to manage all views that a software system provides. A view handler component allows clients to open, manipulate and dispose of views. It also coordinates dependencies between view and organizes their update.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20View%20Handler/'"),
				patternInsert("'Forward-Receiver', ÔDesignÕ,'The Forwarder-Receiver design pattern provides transparent interprocess communication for software systems with a peer-to-peer interaction model. It introduces forwarders and receivers to decouple peers from the underlying communication mechanisms.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Forward-Receiver/'"),
				patternInsert("'Client-Dispatcher-Server', ÔDesignÕ,'The Client-Dispatcher-Server design pattern introduces an intermediate layer between clients and servers, the dispatcher component. It provides location transparency by means of a name service, and hides the details of the establishment of the communication connection between clients and servers.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20ClientDispatcherServer/'"),
				patternInsert("'Publisher-Subscriber', ÔDesignÕ,'The Publisher-Subscriber design pattern helps to keep the state of cooperating components synchronized. To achieve this it enables one-way propagation of changes: one publisher notifies any number of subscribers about changes to its state.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Publisher%20Subscriber/'"),
				patternInsert("'Strategy', ÔDesignÕ,'Define a family of algorithms, encapsulate each one, and make them interchangeable. Strategy lets the algorithm vary independently from clients that use it.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Strategy/'"),
				patternInsert("'Factory', ÔDesignÕ,'Define an interface for creating an object, but let subclasses decide which class to instantiate. Factory Method lets a class defer instantiation to subclasses.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Factory%20Method/'"),
				patternInsert("'Decorator', ÔDesignÕ,'Attach additional responsibilities to an object dynamically. Decorators provide a flexible alternative to subclassing for extending functionality.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Decorator/'"),
				patternInsert("'Composite', ÔDesignÕ,'Compose objects into tree structures to represent part-whole hierarchies. Composite lets clients treat individual objects and compositions of objects uniformly.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Composite/'"),
				patternInsert("'Template Method', ÔDesignÕ,'Define the skeleton of an algorithm in an operation, deferring some steps to subclasses. Template Method lets subclasses redefine certain steps of an algorithm without changing the algorithms structure.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Template%20Method/'"),
				patternInsert("'Command', ÔDesignÕ,'Encapsulate a request as an object, thereby letting you parameterize clients with different requests, queue or log requests, and support undoable operations.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Command/'"),
				patternInsert("'Chain of Responsibility', ÔDesignÕ,'Avoid coupling the sender of a request to its receiver by giving more than one object a chance to handle the request. Chain the receiving objects and pass the request along the chain until an object handles it.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Chain%20of%20Responsability/'"),
				patternInsert("'Facade', ÔDesignÕ,'Provide a unified interface to a set of interfaces in a subsystem. Façade defines a higher-level interface that makes the subsystem easier to user.'" +
				",'','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Facade/'"),
				patternInsert("'Transaction Script', ÔDesignÕ,'Organizes business logic by procedures where each procedure handles a single request from the presentation.'" +
				...
				patternInsert("'Domain Model', ÔDesignÕ,'An object model of the domain that incorporates both behavior and data.','','','Using object-oriented way to handle the business logic. Each object takes a part of the logic that’s relevant to it.','','','EAA Book Online'"),
				patternInsert("'Table Module', ÔDesignÕ,'A single instance that handles the business logic for all rows in a database table or view.','','','','','','EAA Book Online'"),
				patternInsert("'Gateway', ÔDesignÕ,'An object that encapsulates access to an external system or resource.','','','','','',''"),
				patternInsert("'Row Data Gateway', ÔDesignÕ,'An object that acts as a Gateway to a single record in a data source. There is one instance per row.','','','','','','EAA Book Online'"),
				patternInsert("'Active Record', ÔDesignÕ,'An object that wraps a row in a database table or view, encapsulates the database access, and adds domain logic on that data.'" +
				...
				patternInsert("'Table Data Gateway', ÔDesignÕ,'An object that acts as a Gateway to a database table. One instance handles all the rows in the table.A Row Data Gateway gives you objects that look exactly like the record in your record structure but can be accessed with the regular mechanisms of your programming language. All details of data source access are hidden behind this interface.'" +
				",'','','','A Row Data Gateway acts as an object that exactly mimics a single record, such as one database row. In it each column in the database becomes one field. The Row Data Gateway will usually do any type conversion from the data source types to the in-memory types, but this conversion is pretty simple. This pattern holds the data about a row so that a client can then access the Row','','EAA Book Online'"), 		patternInsert("'Application Controller', ÔDesignÕ,'A centralized point for handling screen navigation and the flow of an application.','','','','','','EAA Book Online'"),
				patternInsert("'Transform View', ÔDesignÕ,'A view that processes domain data element by element and transforms it into HTML.','','','','','','EAA Book Online'"),
				patternInsert("'Template View', ÔDesignÕ,'Renders information into HTML by embedding markers in an HTML page.','','','','','','EAA Book Online'"),
				patternInsert("'Two Step View', ÔDesignÕ,'Turns domain data into HTML in two steps: first by forming some kind of logical page, then rendering the logical page into HTML.','','','','','','EAA Book Online'"),
				patternInsert("'Bridge', ÔDesignÕ,'Decouple an abstraction from its implementation so that the two can vary independently.','','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Bridge/'"),
				patternInsert("'Data Mappter', ÔDesignÕ,'A layer of Mappers that moves data between objects and a database while keeping them independent of each other and the mapper itself.'" +
				...
				patternInsert("'Counted Pointer', ÔIdiomÕ,'This idiom makes memory management of dynamically-allocated shared objects in C++ easier. It introduces a reference counter to a body class that is updated by handle objects. ','','','','','','Book Reference'"),
				patternInsert("'Singleton', ÔIdiomÕ,'Ensure a class only has one instance, and provide a global point of access to it.','','','','','','http://vico.org/pages/PatronsDisseny/Pattern%20Singleton/'"),
				patternInsert("'Indented Control Flow', ÔIdiomÕ,'','','','','','','Book Reference'")};
		return toReturn;
	}
	...
	{	
		String[] toReturn = {
				...
				//three-lay subs
				patternDecisionInsertH("'What is the structure of Business Logic Layer', '','SingleChoice','Unresolved','Architecture','No','Pattern',1"),
				patternDecisionInsertH("'What is the structure of Data Source Layer', '','SingleChoice','Unresolved','Architecture','No','Pattern',1"),
				patternDecisionInsertH("'What is the structure of Presentation Layer', '','SingleChoice','Unresolved','Architecture','No','Pattern',1"),

				//layers subs
				patternDecisionInsertH("'What is the number of abstraction levels/layers?', '','SingleChoice','Unresolved','Architecture','No','Pattern',2"),
				patternDecisionInsertH("'What are the names of the layers and their tasks?(Create sub decisions for each layer))', '','SingleChoice','Unresolved','Architecture','No','Pattern',2"),
				patternDecisionInsertH("'What are the interfaces for each layer? (Create sub decisions)', '','SingleChoice','Unresolved','Architecture','No','Pattern',2"),
				patternDecisionInsertH("'How do the layers structure together?', '','SingleChoice','Unresolved','Architecture','No','Pattern',2"),
				patternDecisionInsertH("'How do the layers commnucate to each other?', '','SingleChoice','Unresolved','Architecture','No','Pattern',2"),
				patternDecisionInsertH("'What is the error-handling strategy?', '','SingleChoice','Unresolved','Architecture','No','Pattern',2"),

				//Pipes and Filters
				patternDecisionInsertH("'What is the sequence of processing tasks?', '','SingleChoice','Unresolved','Architecture','No','Pattern',3"),
				patternDecisionInsertH("'What are the data formats to be passed along each pipe?', '','SingleChoice','Unresolved','Architecture','No','Pattern',3"),
				patternDecisionInsertH("'How to implement the filters? (create sub decisions for each filter)', '','SingleChoice','Unresolved','Architecture','No','Pattern',3"),
				patternDecisionInsertH("'How to implement each pipe connection?(create sub decisions for each connection)', '','SingleChoice','Unresolved','Architecture','No','Pattern',3"),
				patternDecisionInsertH("'What are the data formats to be passed along each pipe?', '','SingleChoice','Unresolved','Architecture','No','Pattern',3"),
				patternDecisionInsertH("'What is the error-handling strategy?', '','SingleChoice','Unresolved','Architecture','No','Pattern',3"),		

				//MVC 16
				patternDecisionInsertH("'How to seperate human-computer interaction from core functionality?', '','SingleChoice','Unresolved','Architecture','No','Pattern',5"),
				patternDecisionInsertH("'How to implement change-propagation mechanism?', 'Follow Publisher-Subscriber patter','SingleChoice','Unresolved','Architecture','No','Pattern',5"),
				patternDecisionInsertH("'How to design and implement the views?', '','SingleChoice','Unresolved','Architecture','No','Pattern',5"),
				patternDecisionInsertH("'How to design and implement the controllers?', '','SingleChoice','Unresolved','Architecture','No','Pattern',5"),
				patternDecisionInsertH("'How to implement the view-controller relationship?', '','SingleChoice','Unresolved','Architecture','No','Pattern',5"),
				patternDecisionInsertH("'How to implement the set-up of MVC?', '','SingleChoice','Unresolved','Architecture','No','Pattern',5"),
				patternDecisionInsertH("'How to create view dynamically', '','SingleChoice','Unresolved','Architecture','No','Pattern',5"),
				patternDecisionInsertH("'Pluggable controllers', '','SingleChoice','Unresolved','Architecture','No','Pattern',5"),
				patternDecisionInsertH("'What is the infrastructure for hierarchical views and controllers?', '','SingleChoice','Unresolved','Architecture','No','Pattern',5"),

				//Presentation-Abstraction-Control
				patternDecisionInsertH("'What is the model of the application?', '','SingleChoice','Unresolved','Architecture','No','Pattern',7"),
				patternDecisionInsertH("'What is the general strategy for organizing the PAC hierarchy?', '','SingleChoice','Unresolved','Architecture','No','Pattern',7"),
				patternDecisionInsertH("'What is the top-level PAC agent?', '','SingleChoice','Unresolved','Architecture','No','Pattern',7"),
				patternDecisionInsertH("'What is the bottom-level PAC agents?', '','SingleChoice','Unresolved','Architecture','No','Pattern',7"),
				patternDecisionInsertH("'What is the bottom-level PAC agents for system services?', '','SingleChoice','Unresolved','Architecture','No','Pattern',7"),
				patternDecisionInsertH("'What is the intermediate-level PAC agents to compose lower-level PAC agents?', '','SingleChoice','Unresolved','Architecture','No','Pattern',7"),
				patternDecisionInsertH("'What is the intermediate-level PAC agents to coordinate lower-level PAC agents?', '','SingleChoice','Unresolved','Architecture','No','Pattern',7"),
				patternDecisionInsertH("'How to seperate core functionality from human-computer interaction?', '','SingleChoice','Unresolved','Architecture','No','Pattern',7"),
		};
		return toReturn;
	...
	{	
		String[] toReturn = {
				patternProblemCategoriesInsertH("'From_Mud_To_Structure', 'Architecture'"),
				patternProblemCategoriesInsertH("'Distributed_Systems', 'Architecture'"),
				patternProblemCategoriesInsertH("'Interactive_Systems', 'Architecture'"),
				patternProblemCategoriesInsertH("'Adaptable_Systems', 'Architecture'"),
				patternProblemCategoriesInsertH("'Structural_Decomposition', 'Design'"),
				patternProblemCategoriesInsertH("'Organization_Of_Work', 'Design'"),
				patternProblemCategoriesInsertH("'Access_Control', 'Design'"),
				patternProblemCategoriesInsertH("'Management', 'Design'"),
				patternProblemCategoriesInsertH("'Communication', 'Design'"),
				patternProblemCategoriesInsertH("'Other_Architecture_Problem', 'Architecture'"),
				patternProblemCategoriesInsertH("'Other_Design_Problem', 'Design'") };
		return toReturn;
	}
